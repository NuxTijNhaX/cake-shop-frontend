...
import { capitalize, parseRequestUrl } from "../utils.js";

const CategoryScreen = {
    render: async () => {
        const request = parseRequestUrl();
        const products = await getProducts("", (capitalize(request.id) + " Cake"), request.sub, request.verb);
        return `
            <div class="small-container">

            <div class="row row-2">
                <h2>${(capitalize(request.id) + " Cake")}</h2>
                <select id="order-by">
